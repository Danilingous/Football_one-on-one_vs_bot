...
public class FieldShopButton : MonoBehaviour
{
    ...
    public void SetStatus(bool isBuyed, bool isEquipped)
    {
        ...
        if (!isBuyed)
        {
            ...
            if (ServiceLocator.CurrentSericeLocator.GetServise<GameParametrs>().CountFieldAvaliable + 1 < _buttonNumber)
            {
    ...
    public void Click()
    {
        if (!_isBuyed)
        {
            if (_buttonNumber - 1 == ServiceLocator.CurrentSericeLocator.GetServise<GameParametrs>().CountFieldAvaliable)
            {
                if (ServiceLocator.CurrentSericeLocator.GetServise<CoinCounter>().TrySpendCoins(_cost))
                {
                    ServiceLocator.CurrentSericeLocator.GetServise<GameParametrs>().AddAvaliableField();
        ...
        {
            ServiceLocator.CurrentSericeLocator.GetServise<GameParametrs>().ChangeChoosenField(_buttonNumber - 1);
        }

        ServiceLocator.CurrentSericeLocator.GetServise<DuttonsFieldController>().SetStatusButtons();
    }
}
